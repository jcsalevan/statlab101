...
#' @return This function returns either a graphical display of a permutation
#' test of Pearson's correlation between x and y, when plot = T, or the
#' probability of obtaining the correlation between x and y.
...
permCor <- function(x, y, n_samp = 10000, plot = T){
    ...
    if (plot == T){
        ...
        hist(corResults, col = "yellow", main = "", xlab = "Correlations",
             breaks = 50, xlim = range(corResults,cor(x,y)))
